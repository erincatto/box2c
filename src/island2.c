...
b2Island2* b2CreateIsland2(b2IslandBuilder* builder, int32_t islandIndex, struct b2World* world, const b2TimeStep* step)
{
	...
	b2ContactSolverDef2 contactSolverDef;
	...
	island->contactSolver = b2CreateContactSolver2(alloc, &contactSolverDef);
...
void b2DestroyIsland2(b2Island2* island)
{
	...
	b2DestroyContactSolver2(alloc, island->contactSolver);
...
void b2SolveIsland2(b2Island2* island)
{
	...
	// Solver data
	b2ContactSolver_Initialize2(island->contactSolver);
	...
	for (int32_t i = 0; i < step->velocityIterations; ++i)
	{
		...
		b2ContactSolver_SolveVelocityConstraints2(island->contactSolver);
	}
	b2TracyCZoneEnd(velc);

	// Special handling for restitution
	b2ContactSolver_ApplyRestitution2(island->contactSolver);

	// Store impulses for warm starting
	b2ContactSolver_StoreImpulses2(island->contactSolver);
	...
	{
		bool contactsOkay = b2ContactSolver_SolvePositionConstraintsBlock2(island->contactSolver);
